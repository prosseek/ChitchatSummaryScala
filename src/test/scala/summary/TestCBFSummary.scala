...
class TestCBFSummary extends FunSuite {
  ...
  test ("Simple") {
    ...
    val fbf = CBFSummary(filePath=simpleFile, q = 1*8, typeInference = typeInference)
  ...
  test ("load") {
    val saveFilePath = "./src/test/resources/jsonFiles/simple/simple_cbf_q4.bin"
    val typeInference = TypeInference() // () to use apply
    val fbf = CBFSummary(filePath=saveFilePath, q = 0, typeInference = typeInference)
  ...
  test ("byte array") {
    ...
    val bf = CBFSummary(input = m, q = 4*8, typeInference = ti)
