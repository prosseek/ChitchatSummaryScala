...
class TestFBFSummary extends FunSuite {
  ...
  test ("Simple") {
    ...
    val fbf = FBFSummary(filePath=simpleFile, q = 2*8, typeInference = typeInference)
  ...
  test ("load") {
    val saveFilePath = "./src/test/resources/jsonFiles/simple/simple_fbf_q4.bin"
    val typeInference = TypeInference() // () to use apply
    val fbf = FBFSummary(filePath=saveFilePath, q = 2*8, typeInference = typeInference)
  ...
  test ("byte array") {
    ...
    val bf = FBFSummary(input = m, q = 8*2, typeInference = ti)
